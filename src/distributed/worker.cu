...
// Listens on TCP, renders assigned horizontal tile per frame, sends pixels back.
//
...
int main(int argc, char** argv) {
    ...
    fprintf(stderr, "Worker | tile %dx%d (rows %d..%d) spp=%d depth=%d\n",
            tile_w, tile_h, req.row_start, req.row_end, req.spp, req.depth);
    ...
    while (true) {
        ...
        // Copy result to host
        cudaMemcpy(h_rgba.data(), ts.d_rgba,
                   tile_w * tile_h * sizeof(uchar4),
        ...
        if (!send_all(conn, &resp, sizeof(resp))) break;
        if (!send_all(conn, h_rgba.data(),
                      resp.num_pixels * sizeof(uchar4))) break;
    }
