// Serial real-time ray tracer with GLFW window.
// Usage: ./bin/serial_realtime_rt [--scene random|cornell] [--width W]
//                                  [--spp N] [--depth D]
// Defaults: 400x225, 1 spp, 3 depth.  ESC or close to quit.
...
int main(int argc, char** argv) {
    ...
    GLFWwindow* window = glfwCreateWindow(W, H, "Ray Tracer (Serial)", nullptr, nullptr);
    ...
    fprintf(stderr, "Serial | scene=%s  res=%dx%d  spp=%d  depth=%d  | ESC to quit\n",
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (s >= 0.5) {
            ...
            snprintf(title, sizeof(title),
                     "Ray Tracer (Serial)  |  %.2f FPS  |  spp=%d  depth=%d  |  %dx%d",
